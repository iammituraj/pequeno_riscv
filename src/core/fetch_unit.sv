...
always_ff @(posedge clk or negedge aresetn) begin
   ...
   // Out of reset
      ...
      else if (bp_or_ras_flush && !stall) begin instr_valid_rg[0] <= 1'b0 ; end              // Invalidate on BP/RAS flush IFF not stalling - cz the instr that generated the flush should not get invalidated in the pipeline!
...
ras_predictor #(
   ...
   .i_st_rbk_en   ('0),//TBD: logic to be added, I/P port of FU
   .i_st_rbk_ptr  ('0),//TBD: logic to be added, I/P port of FU
   .i_st_rbk_cnt  ('0),//TBD: logic to be added, I/P port of FU

   .i_is_call_fu  ('0),//TBD: logic to be added, I/P port of FU
   .i_is_ret_fu   ('0),//TBD: logic to be added, I/P port of FU
   .i_is_call_du  ('0),//TBD: logic to be added, I/P port of FU
   .i_is_ret_du   ('0),//TBD: logic to be added, I/P port of FU
   ...
   .i_instr_valid (instr_valid & ~bu_or_bp_flush),  // BU/BP flush should immediately invalidate to avoid any spurious RAS flush/push/pop in the next clock cycle...
                                                    // RAS flush need not invalidate, as it wont cause another spurious RAS flush in the next clk cycle...
...
`ifdef DBG
// Debug Interface
assign o_fu_dbg = {bp_flush, (is_op_branch & instr_valid), (is_op_jal & instr_valid)} ;//TBD: add RAS flush debug port, CALL, RET...
`endif
