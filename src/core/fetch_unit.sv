...
//----%%                    # Sends the fetched instruction and branch, BP/RAS prediction signals as payload to Decode Unit (DU).
...
logic [`XLEN-1:0] bp_pc              ;  // PC to branch to on BP flush
...
logic [`XLEN-1:0] ras_ret_addr_cmb     ;  // RAS predicted RET addr, combinational
logic             ras_ret_taken        ;  // RAS predicted RET taken status
logic             ras_ret_taken_cmb    ;  // RAS predicted RET taken status, combinational
`endif
...
logic             any_flush       ;  // Any flush
logic             fu_flush_ext    ;  // External flush generated by FU to upstream pipeline

//==================================================================================================
// Next PC logic
//==================================================================================================
logic [`XLEN-1:0] nxt_pc    ;  // Next PC
logic [`XLEN-1:0] pc2imem   ;  // PC to IMEMIF
logic [`XLEN-1:0] bp_or_ras_pc, bp_or_ras_pc_rg ;  // BP/RAS branch PC

`ifdef EN_IMEM_FLUSH_BYP
//--------------------------------------------------------------------------------------------------
// Flush bypass to IMEMIF
//--------------------------------------------------------------------------------------------------
logic [`XLEN-1:0] nxt_pc_pre, nxt_pc_pre_plus4;  // Next PC pre-processed
logic [2:0]       nxt_pc_sel;                    // Priority encoded PC select
// Priority-encoded PC select
`ifdef RAS
always_comb begin
   casez ({i_exu_bu_flush, bp_flush, ras_flush, !i_imem_stall})
      4'b1???  : nxt_pc_sel = 3'd0;  // EXU-BU flush, highest priority
      4'b01??  : nxt_pc_sel = 3'd1;  // Branch predictor flush
      4'b001?  : nxt_pc_sel = 3'd2;  // RAS flush; RAS & BP flushes are exclusive, so the priority doesn't matter...
      4'b0001  : nxt_pc_sel = 3'd3;  // Request PC+4
      default  : nxt_pc_sel = 3'd4;  // Hold; stalled with no flush
   endcase
end
`else
always_comb begin
   casez ({i_exu_bu_flush, bp_flush, !i_imem_stall})
      3'b1??  : nxt_pc_sel = 3'd0;  // EXU-BU flush, highest priority
      3'b01?  : nxt_pc_sel = 3'd1;  // Branch predictor flush
      3'b001  : nxt_pc_sel = 3'd2;  // Request PC+4
      default : nxt_pc_sel = 3'd4;  // Hold; stalled
   endcase
end
`endif

// PC mux, combinational; selects next PC based on the priority-encoded select above
`ifdef RAS
always_comb begin
   case (nxt_pc_sel)
      3'd0    : nxt_pc_pre = i_exu_bu_pc    ;  // BU PC
      3'd1    : nxt_pc_pre = bp_or_ras_pc_rg;  // BP PC
      3'd2    : nxt_pc_pre = bp_or_ras_pc_rg;  // RAS PC
      default : nxt_pc_pre = pc_rg          ;  // Hold
   endcase
end
`else
always_comb begin
...
// BU/BP/RET/PC+4
assign nxt_pc_pre_plus4 = nxt_pc_pre + `XLEN'(4);

// Next PC = PC if stalling
assign nxt_pc  = nxt_pc_sel[2]? pc_rg : nxt_pc_pre_plus4 ;

// IMEM is addressed by the registered PC as long as no flush
// Bypass PC to IMEMIF with Branch PC if flush happened
assign pc2imem = any_flush ? nxt_pc_pre : pc_rg ;

`else
//--------------------------------------------------------------------------------------------------
// No flush bypass to IMEMIF
//--------------------------------------------------------------------------------------------------
`ifdef RAS
// PC mux, combinational
always_comb begin
   casez ({i_exu_bu_flush, bp_flush, ras_flush, !i_imem_stall})
      4'b1???  : nxt_pc = i_exu_bu_pc      ;  // EXU-BU flush, highest priority
      4'b01??  : nxt_pc = bp_or_ras_pc_rg  ;  // Branch predictor flush
      4'b001?  : nxt_pc = bp_or_ras_pc_rg  ;  // RAS flush; RAS & BP flushes are exclusive, so the priority doesn't matter...
      4'b0001  : nxt_pc = pc_rg + `XLEN'(4);  // Request PC+4
      default  : nxt_pc = pc_rg            ;  // Hold; stalled with no flush
   endcase
end
`else
// PC mux, combinational
always_comb begin
   casez ({i_exu_bu_flush, bp_flush, !i_imem_stall})
      3'b1??  : nxt_pc = i_exu_bu_pc      ;  // EXU-BU flush, highest priority
      3'b01?  : nxt_pc = bp_or_ras_pc_rg  ;  // Branch predictor/RAS flush
      3'b001  : nxt_pc = pc_rg + `XLEN'(4);  // Request PC+4
      default : nxt_pc = pc_rg            ;  // Hold; stalled
   endcase
end
`endif

// IMEM is always addressed by the registered PC
assign pc2imem = pc_rg ;
`endif
...
always_ff @(posedge clk or negedge aresetn) begin
   // Reset
   if (!aresetn) begin
      pc_rg       <= PC_INIT-`XLEN'(4);  // PC_INIT-4, so that in the very next valid cycle, PC become PC_INIT
      pc_valid_rg <= 1'b0 ;
   end
   // Out of reset
   else begin
...
// Generates RET taken status, which is checked during RET resolution at Execution Unit (EXU)
// Generates RET address for RAS flush
//==================================================================================================
generate
...
ras_predictor #(
   ...
   .o_ret_addr_cmb    (ras_ret_addr_cmb),
   .o_ret_taken       (ras_ret_taken),
   .o_ret_taken_cmb   (ras_ret_taken_cmb),
   .o_flush           (ras_flush)
);
...
//==================================================================================================
// Merged BP/RAS branch PC - branch_taken and ret_taken are mutually exclusive
//==================================================================================================
`ifdef RAS
assign bp_or_ras_pc = ras_ret_taken_cmb ? ras_ret_addr_cmb : bp_pc ;
`else
assign bp_or_ras_pc = bp_pc ;
`endif
// No reset for better PPA
always_ff @(posedge clk) begin
   if (!stall) begin
      bp_or_ras_pc_rg <= bp_or_ras_pc ;
   end
end
...
`ifdef RAS
assign any_flush       = flush | bp_or_ras_flush ;
`else
assign any_flush       = flush | bp_flush ;
`endif
assign fu_flush_ext    = any_flush    ;  // Any flush should be propagated to the upstream...
...
assign o_du_ras_ret_addr  = bp_or_ras_pc_rg ;  // Merged register; safe since EXU only reads it when ras_ret_taken=1
